...
def group_by_umbrella(xyz, new_xyz, k=9, cuda=False):
    ...
    torch.cuda.empty_cache() # group_xyz [64,512,4,3], 4应该是除了self以外的neighbours
...
class TriangleSurfaceConstructor(nn.Module):
    ...
    def __init__(self, k, in_channel, aggr_type='sum', return_dist=False, random_inv=True, cuda=False):
        super(TriangleSurfaceConstructor, self).__init__()
        self.k = k
        self.return_dist = return_dist
        self.random_inv = random_inv
        self.aggr_type = aggr_type
    ...
    def forward(self, center):
        ...
        # centroids [64,512,3]
        group_center = cal_center(group_xyz)
        if self.return_dist: # [64,512,1]
            # positions = sum(normals*centroids, dim=2)/sqrt(3) 位置编码？
            group_pos = cal_const(group_normal, group_center)
            # 去除 group_normal,group_center,group_pos中 Nan
            # 注意这里使用check_nan，如果使用check_nan_umb会发生错误
            group_normal, group_center, group_pos = check_nan(group_normal, group_center, group_pos)
